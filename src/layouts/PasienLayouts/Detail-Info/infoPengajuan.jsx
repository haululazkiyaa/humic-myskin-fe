...
import { getDetailPengajuan } from "../../../api/api";
...
const InfoPengajuan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(getDetailPengajuan(id));
        setLoading(false);
  ...
  return (
    <div className="py-20 lg:py-32 w-full px-4 lg:px-32">
      {loading ? (
        <h1 className="w-full text-center mt-">Loading...</h1>
      ) : (
          ...
          <div className="text-center flex flex-col items-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 mt-4 gap-4 w-full">
              ...
              <div className="lg:col-span-2 p-4 text-left rounded-lg shadow-md">
                ...
                <p>
                  Saya pertama kali menyadari adanya perubahan pada tahi lalat
                  di punggung saya sekitar enam bulan yang lalu. Awalnya, tahi
                  lalat tersebut hanya sedikit lebih besar dari biasanya, tetapi
                  seiring waktu, ukurannya bertambah dan warnanya berubah
                  menjadi lebih gelap, hampir hitam. Saya juga mulai merasakan
                  gatal di area tersebut, dan kadang-kadang benjolan itu
                  berdarah tanpa alasan yang jelas
                </p>
              </div>
            </div>
            <ResultDetect />
